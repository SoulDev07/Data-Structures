...
// Converts given expression to prefix expression
void toPrefix(Stack *stack, char arr[CHAR_ARR_SIZE], char prefix[CHAR_ARR_SIZE])
{
    char ch, temp;
    ...
    for (int i = size - 1; i >= 0; i--)
    {
        ...
        {
            // Pop all operators till corresponding closing parenthesis is reached or stack is empty
        ...
        {
            // For operators having higher than precedence to current operator,
            // Pop them and display it to output till the condition if false or
            // closing parenthesis is reached or stack is empty
...
int main()
{
    ...
    Stack *stack = create(size); // Create stack
